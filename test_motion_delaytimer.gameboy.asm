;speed is handled by checking the speed value against a counter
;this allows us to divide the max speed by up to 126 - much smoother motion than by skipping pixels
;acceleration is sort of implemented, but I just realized that we need to handle speed as a vector as well,
;or at least figure out a way to handle it differnetly
...
;can be +126 or -126, with 127 being 0. closer to 0 = faster 
...
DEF divider_counter EQU $C006

DEF accel_divider_counter EQU $C007
;sprite acceleration will be treated as a vector, 
;with its direction affected by the dpad(and hopefully friction and gravity eventually)
DEF sprite_accel_direction EQU $C008
DEF sprite_accel_magnitude EQU $C009

DEF bit_dir_accel_x EQU %00000001 ;1 is right, 0 is left
DEF bit_dir_accel_y EQU %00000010 ;1 is down, 0 is up
...
Setup:
	...
	;init divider counter 0
	ld a, 0
	ld [divider_counter], a

	;init acceleration vector to 0, in the -x -y direction
	ld a, 0
	ld [sprite_accel_magnitude], a
	ld [sprite_accel_direction], a
	ld [accel_divider_counter], a
	...
	;110 = ~65kHz
	;111 = ~17kHz
	ld a, bit_timer_start | bit_timer_clocksel_0; ~65kHz
	ld [rTAC], a
...
goRight:
	ld a, [sprite_accel_direction]
	set bit_dir_accel_x, a
	ld [sprite_accel_direction], a
	ld a, 255
	ld [sprite_accel_magnitude], a
	ret
...
updateSpriteSpeed:
	ld a, [sprite_accel_magnitude]
	cp 0
	jp nz, accelerate
	ret
accelerate:
	ld a, [sprite_accel_direction]
	bit bit_dir_accel_x, a
	jp nz, accelerateRight
	ret
accelerateRight:
	ld a, [sprite_accel_magnitude]
	ld b, a
	ld a, [accel_divider_counter]
	cp b
	jp z, incSpeedX
	inc a
	ld [accel_divider_counter], a
...
updateSpritePos:
	ld a, [sprite_speed_x]
	sub 127
	ret z ;don't move if speed is 0(127)
	jp nc, moveRight
	jp c, moveLeft
	ret
moveRight:
	ld b, a
	ld a, [divider_counter]
	cp b ;check if the divider count has reached the speed value
	jp z, incPosX
	inc a ;if not, increment the counter and return
	ld [divider_counter], a
	ret
incPosX:
	ld a, [sprite_pos_x]
	inc a ;move one pixel to the right
	ld [sprite_pos_x], a
	ld a, 0 ;reset the counter to 0
	ld [divider_counter], a
	ret
moveLeft:
	cpl ;flip the bits so 255 => 0 etc
	inc a ;we want to start at 1
	ld b, a
	ld a, [divider_counter]
	cp b ;check if the divider count has reached the speed value
	jp z, decPosX
...
decPosX:
	ld a, [sprite_pos_x]
	dec a ;move one pixel to the left
	ld [sprite_pos_x], a
	ld a, 0 ;reset the counter to 0
	ld [divider_counter], a
	ret
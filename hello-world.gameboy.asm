...
DEF scrollVal EQU $C000
DEF scrollSpeed EQU $C001
DEF buttons EQU $C003
...
EntryPoint:
	...
	;ld a, $00
	;ld [rIF], a
	ld a, $04 ;timer overflow interrupt
	;or $01 ;vertical blanking interrupt(don't use yet)
	;or $10 ;button input interrupt
	ld [rIE], a
	ei ;master interrupt enable

	;set timer overflow modulo
	ld a, 0
	ld [rTMA], a

	;start the timer
	ld a, $04
	;or $01 ;f/2^4 input clock
	ld [rTAC], a 
...
CopyTiles:
	...
	ld a, b
	or a, c
	jp nz, CopyTiles ;if (a | c) == 0, jump to CopyTiles
...
;this seems to just cause everything else to be ignored since it's called constantly
;SECTION "Vertical Blanking Interrupt", ROM0[$040]
;	jp Update_Scroll

;SECTION "Button Input Interrupt", ROM0[$060]
;	di
;	jp Inc_Speed
;	reti
